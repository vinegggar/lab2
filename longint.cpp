...
Lint Lint::operator*(Lint other) {
    return Lint(multer->multiply(digits, other.digits));
}

void Lint::getInv() { cout << Decimal(digits).inverse(); }

Lint Lint::operator/(Lint other) {
    return Lint((Decimal(digits) / Decimal(other.digits)).getDigits());
}

Lint Lint::operator%(Lint other) {
    Lint res;
    if (other.digits == vector<int>{1} or digits == other.digits)
        return Lint("0");
    if (*this < other)
        return *this;
...
bool MillerRabin::isPrime(vector<int> d, int iters) {
    ...
    for (int i = 0; i < iters; ++i) {
        Lint rand = Lint::genRandLint(Lint("2"), Lint(d) - Lint("1"));
        Lint randPowMod = rand.powMod(Lint(k), Lint(d));

        if (randPowMod == Lint("1") || randPowMod == Lint(d) - Lint("1"))
            continue;

        for (int j = 1; j < s; ++j) {
            randPowMod = (randPowMod * randPowMod) % Lint(d);

            if (randPowMod == Lint("1"))
                return false;

            if (randPowMod == Lint(d) - Lint("1"))
                break;
        }
...
int Lint::jacobi(Lint other) {
    ...
    while (a != Lint("0")) {
        while (a.digits[0] % 2 == 0) {
            a = a / Lint("2");
            r = b % Lint("8");
            if (r == Lint("3") || r == Lint("5"))
                t = -t;
        }
        r = b;
        b = a;
        a = r;
        if (a % Lint("4") == Lint("3") && b % Lint("4") == Lint("3"))
            t = -t;
        a = a % b;
    }
    if (b == Lint("1"))
