...
vcd to_complex(vector<int> &d) {
    vcd res;
    for (int i : d)
        res.push_back(complex<double>(i, 0));
    return res;
}
...
vcd fft(vcd d, cd w) {
    ...
    vcd even_fft = fft(even, w * w);
    vcd odd_fft = fft(odd, w * w);
    vcd res(d.size());
    cd wp(1, 0);
...
vector<int> fft_mul(vector<int> d1, vector<int> d2) {
    ...
    d1.resize(next_pow2(prod_size));
    d2.resize(next_pow2(prod_size));
...
vector<int> toom_cook_mul(vector<int> d1, vector<int> d2) {
    ...
    v_1 = toom_cook_mul(x0 - x1 + x2, y0 - y1 + y2), two = {2}, four = {4},
    three = {3},
    v2 = toom_cook_mul(x0 + two * x1 + four * x2, y0 + two * y1 + four * y2),
    v_inf = toom_cook_mul(x2, y2),

    // interpolation process...
    t1 = divn((three * v0 + two * v_1 + v2), 6) - two * v_inf, t2 = divn((v1 + v_1), 2);
...
vector<vector<int>> factorize(vector<int> n) {
    vector<int> s(1), k, n_1 = n - vector<int>{1};
    carry_res(n_1);
    trim(n_1);
    while (n_1[0] % 2 == 0) {
        s[0] += 1;
